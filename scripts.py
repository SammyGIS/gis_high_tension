...
def col_angle_relative_severity(value, thresholds):
...
def sep_distance_relative_severity(value, thresholds):
    if value < thresholds[0]:
        return "High"
...
def calculate_collocation_info(pipeline_gdf, high_tension_gdf, buffer_distance,
    ...
    pipeline_buffer_gdf['rs_colLength'] = pipeline_buffer_gdf['col_length'].apply(col_length_relative_severity, thresholds=collocation_thresholds)
    pipeline_buffer_gdf['rs_colAngle'] = pipeline_buffer_gdf['col_angle'].apply(col_angle_relative_severity, thresholds=angle_thresholds)
    pipeline_buffer_gdf['rs_sepDist'] = pipeline_buffer_gdf['sep_dist'].apply(sep_distance_relative_severity, thresholds=separation_thresholds)
    
        
    # Select only the desired columns
    final_columns = ['PIPELINE_I', 'col_length', 'col_angle', 'sep_dist',
                     'rs_colLength','rs_colAngle','rs_sepDist','geometry']
...
if __name__ == "__main__":
    ...
    collocation_thresholds = [1500, 300]
    separation_thresholds = [30, 152, 300]  # Adjust separation thresholds as needed
